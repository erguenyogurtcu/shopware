...
class CacheInvalidationSubscriber
{
    ...
    public function invalidateSnippets(EntityWrittenContainerEvent $event): void
    {
        ...
        if (Feature::isActive('cache_rework')) {
            $this->cacheInvalidator->invalidate(['shopware.theme', 'shopware.translator']);
    ...
    public function invalidateNavigation(EntityWrittenContainerEvent $event): void
    {
        if (!Feature::isActive('cache_rework')) {
            return;
        }

        $tags = $this->getChangedEntryPointsNew($event);

        $included = $this->getChangedNavigationCategories($event);

        $tags = array_merge($tags, $included);
    ...
    {
        // todo@skroblin changing one category produces a lot of this change events, why??
    ...
    public function invalidateListingRouteByCategoryIds(CategoryIndexerEvent $event): void
    {
        ...
        // todo@skroblin only invalidate when product mapping, cms config changed
    ...
    public function invalidateDetailRoute(ProductChangedEventInterface $event): void
    {
        ...
        $this->cacheInvalidator->invalidate(
            array_map(ProductDetailRoute::buildName(...), [...$parentIds, ...$event->getIds()])
        );
    }
    ...
    private function getChangedEntryPointsNew(EntityWrittenContainerEvent $event): array
    {
        ...
        $footer = $event->getPrimaryKeysWithPropertyChange(SalesChannelDefinition::ENTITY_NAME, ['footerCategoryId']);

        $service = $event->getPrimaryKeysWithPropertyChange(SalesChannelDefinition::ENTITY_NAME, ['serviceCategoryId']);

        return array_merge(
            array_map(NavigationRoute::headerName(...), $header),
            array_map(NavigationRoute::footerName(...), $footer),
            array_map(NavigationRoute::serviceName(...), $service),
        );
    }
    ...
    private function getChangedNavigationCategories(EntityWrittenContainerEvent $event): array
    {
        ...
        $paths = $this->connection->fetchFirstColumn(
            'SELECT path FROM category WHERE id IN (:ids)',
            ['ids' => Uuid::fromHexToBytesList($categories)],
            ['ids' => ArrayParameterType::BINARY]
        );

        $entries = $this->connection->fetchAllAssociative(
            'SELECT LOWER(HEX(navigation_category_id)) as header,
                    LOWER(HEX(footer_category_id)) as footer,
                    LOWER(HEX(service_category_id)) as service
             FROM sales_channel',
            ['ids' => Uuid::fromHexToBytesList($categories)],
            ['ids' => ArrayParameterType::BINARY]
        );
        ...
        foreach ($entries as $entry) {
            $all = array_merge($all, array_filter($entry));
        }
        ...
        foreach ($paths as $path) {
            ...
            $contains = array_intersect($path, $all);

            if (!empty($contains)) {
                $invalidate = array_merge($invalidate, array_map(NavigationRoute::category(...), $contains));
            }
        }
    ...
    private function getProductCategoryIds(array $ids): array
    {
        ...
        return $this->connection->fetchFirstColumn(
            'SELECT DISTINCT LOWER(HEX(category.id))
             FROM cms_page
                INNER JOIN category ON category.cms_page_id = cms_page.id
                INNER JOIN cms_section ON cms_section.cms_page_id = cms_page.id
                INNER JOIN cms_block ON cms_block.cms_section_id = cms_section.id
                INNER JOIN cms_slot ON cms_slot.cms_block_id = cms_block.id
             WHERE cms_slot.type = :type
             AND category.active = 1
             AND category.id IN (:ids)',
            ['type' => 'product-listing', 'ids' => Uuid::fromHexToBytesList($ids)],
            ['ids' => ArrayParameterType::BINARY]
        );
    }
